...
import { Check, CornerDownLeft, Cross, Mic, Paperclip, Pencil, RefreshCw, Trash, X } from 'lucide-react';
			...
			<ExpandableChatBody className='max-h-[50vh] flex-grow overflow-scroll'>
				<ChatMessageList smooth={true}>
					{groupedMessages.map((item, index) => {
						...
						const variant = item.created_by === user?.uuid ? 'sent' : 'received';
						...
						return (
							<ChatBubble
								key={item.uuid}
								variant={variant}
								className={shouldBlur ? (shouldBlurOut ? 'blur-out' : 'blur') : undefined}
							>
								<ChatBubbleAvatar fallback={item.page === 'repair' ? 'R' : 'D'} />
								<ChatBubbleMessage
									isLoading={form.formState.isSubmitting}
									className={item.created_by === user?.uuid ? 'bg-primary' : 'bg-secondary'}
								>
									{item.message}
									<br />
									<div className='flex items-center justify-end gap-2'>
										<span className='mt-2 text-right text-xs'>{item.created_by_name}</span>
										<ChatBubbleTimestamp
											timestamp={format(new Date(item?.created_at || ''), 'hh:mm')}
										/>

										{item.updated_at && <span className='mt-2 text-right text-xs'>(Edited)</span>}
									</div>
								</ChatBubbleMessage>
								<ChatBubbleActionWrapper>
			...
			<ExpandableChatFooter className='shrink-0'>
				<form className='relative rounded-lg border bg-background p-1 focus-within:ring-1 focus-within:ring-ring'>
					<ChatInput
						...
						autoComplete='on'
						onKeyDown={editingMessageId ? (e) => handleEditKeyDown(e, editingMessageId) : onKeyDown}
